...
import { FileText, Users, ChartLineUp, Code, GearSix, Question } from '@phosphor-icons/react';
...
export default function ChatbotDetailLayout({
  ...
  const tabs = [
    ...
    { name: 'Review Queue', href: `${baseUrl}/review`, icon: Question, premium: true, feature: 'knowledgeGap' },
          ...
          {tabs.map((tab) => {
            ...
            return (
              ...
              >
                ...
                {tab.name}
                {!access.allowed && <span className="text-xs text-amber-600/80">✦</span>}
              </Link>
            );
          })}
        </nav>
